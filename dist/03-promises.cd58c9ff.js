...
function onClick(userDelay) {
    for(let i = 0; i < amountOfPromises.value; i += 1)setTimeout(()=>{
        createPromise(i, delayStep.value * i).then(({ position , delay  })=>{
            console.log(`✅ Fulfilled promise ${position} in ${Number(delay) + Number(userDelay)}ms`);
            (0, _notiflixDefault.default).Notify.success(`✅ Fulfilled promise ${position} in ${Number(delay) + Number(userDelay)}ms`);
        }).catch(({ position , delay  })=>{
            console.log(`❌ Rejected promise ${position} in ${Number(delay) + Number(userDelay)}ms`);
            (0, _notiflixDefault.default).Notify.failure(`❌ Rejected promise ${position} in ${Number(delay) + Number(userDelay)}ms`);
        });
    }, userDelay);
}
